...
router.post(
	...
	async (req, res) => {
		...
		try {
			...
			if (checkuser) {
				//if user exists then we will send email exixts msg
			...
			const authToken = jwt.sign(data, process.env.JWT_SECRET); //here token generated
	...
	async (req, res) => {
		...
		try {
			...
			if (!user) {
				...
				return res
					.status(400)
					.json({ error: "Incorrect credentials. please check once again" });
			}
			...
			if (!comparePassword) {
				...
				return res.status(400).json({
					success,
					error: "Incorrect credentials. please check once again",
				});
			}
